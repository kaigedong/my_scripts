...
![输入图片说明](聊聊Linux五种IO模型.assets/20144245_Wtld.png)
...
![输入图片说明](聊聊Linux五种IO模型.assets/20150405_VKYH.png)
...
![输入图片说明](聊聊Linux五种IO模型.assets/20152818_DXcj.png)
...
对于多路复用，也就是轮询多个socket。`多路复用既然可以处理多个IO，也就带来了新的问题，多个IO之间的顺序变得不确定了`，当然也可以针对不同的编号。具体流程，如下图所示：
 ...
 ![输入图片说明](聊聊Linux五种IO模型.assets/20164149_LD8E.png)
...
![输入图片说明](聊聊Linux五种IO模型.assets/21091434_DsZb.png)
...
![输入图片说明](聊聊Linux五种IO模型.assets/20175459_gtgw.png)
...
![https://static.oschina.net/uploads/img/201604/20183314_5rY1.png](聊聊Linux五种IO模型.assets/20183314_5rY1.png)



![输入图片说明](聊聊Linux五种IO模型.assets/20183327_wIGf.png)
...
![输入图片说明](聊聊Linux五种IO模型.assets/21095604_vhHX.png)

通过上面的图片，可以发现non-blocking IO和asynchronous IO的区别还是很明显的。`在non-blocking IO中，虽然进程大部分时间都不会被block，但是它仍然要求进程去主动的check`，并且当数据准备完成以后，也需要进程主动的再次调用recvfrom来将数据拷贝到用户内存。而asynchronous IO则完全不同。`它就像是用户进程将整个IO操作交给了他人（kernel）完成，然后他人做完后发信号通知`。在此期间，`用户进程不需要去检查IO操作的状态，也不需要主动的去拷贝数据`。



+ 搬运自：https://www.jianshu.com/p/486b0965c296